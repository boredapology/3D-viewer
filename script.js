...
const researchData = {
    single: {
        ...
        workflow: {
            ...
            methodDescription: "단일 시점 차량 이미지를 생성한 뒤, 이를 기반으로 3D 모델을 생성한 일반적인 3D 생성 기준 사례입니다. 입력 이미지에는 전면·측면 정보만 포함되며, 후면 및 비가시 영역은 3D 생성 과정에서 추론에 의존합니다.",
            evaluationPoint: "입력에 없는 후·측면 정보가 3D 결과에서 어떻게 보완되는지, 그리고 그 과정에서 어떤 형태 생략이나 구조적 한계가 나타나는지 확인해 주세요."
        },
        subModels: [
            {
                id: "suv",
    ...
    multi: {
        ...
        workflow: {
            ...
            methodDescription: "전면·측면·후면 정보를 함께 포함한 멀티뷰 차량 이미지를 생성한 뒤, 이를 기반으로 3D 모델을 생성한 전략입니다. 단일 시점 입력에서 부족했던 후면 및 비가시 영역 정보를 함께 제공하여, 시점 간 조형 일관성과 후면 디자인 반영을 높이는 것을 목표로 합니다.",
